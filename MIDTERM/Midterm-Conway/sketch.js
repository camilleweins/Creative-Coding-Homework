...
function draw() {
  ...
  for (var i = 0; i < howwide; i++) {
    for (var j = 0; j < howtall; j++) {
      ...
      if(p6==1) // lower left is alive
      {
        // if four or three live neighbors, keep alive; otherwise die.
        if(neighbors==4 || neighbors==3) result = 1; //else result = 0;

      }
      else // lower left pixel is DEAD
      {
        // if exactly 2 live neighbors, become alive; otherwise stay dead.
